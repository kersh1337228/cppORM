...
Model<Types...>* parse(std::vector<std::string> const& row, std::index_sequence<Idxs...>);
template <typename... Types>
...
public:
    ...
    static std::vector<Model*> read(const std::map<std::string, std::string>& data) {
        sqlite3* db;
        sqlite3_open("db.sqlite3", &db);
        ...
        for (auto row : rows)
            requests.emplace_back(unpack<Types...>(
                std::stoi(row[0]), std::vector<std::string>(row.begin() + 1, row.end())
            ));
...
template<typename... Types, size_t... Idxs>
Model<Types...>* parse(std::vector<std::string> const& row, std::index_sequence<Idxs...>){
    return new Model(recast<Types>(row[Idxs])...);
}
template <typename... Types>
Model<Types...>* unpack(unsigned int id, std::vector<std::string> const& row) {
    Model<Types...>* model = parse<Types...>(row, std::make_index_sequence<sizeof...(Types)>{});
    model->id = id;
    return model;
}
