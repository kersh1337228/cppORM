...
public:
    void print_requests(std::vector<Request> const& requests) const {
        std::cout << requests[0].header() << std::endl;
        for (auto request : requests) std::cout << request.print() << std::endl;
    };
    virtual void read_requests() const = 0;
...
public:
    ...
    void read_requests() const final {
        this->print_requests(Request::read({
            {"customer", std::to_string(this->userid)},
        }));
    }
};
